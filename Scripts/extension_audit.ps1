...
.PARAMETER MaliciousExtensionsUrl
    ...
    Default: "https://raw.githubusercontent.com/loog4j/Extension-Audit/main/ExtensionIDs/malicious_extensions.txt"
...
.NOTES
    Requires:     Administrator privileges on first run (to create Event Log source)
    Version:      2.0
    Event ID:     9191 (Information for benign, Warning for malicious)

    Malicious Extensions List Format:
        - Plain text file, one extension ID per line
        - Lines starting with # are treated as comments
        - Blank lines are ignored
        - Chromium extension IDs are 32 lowercase letters (a-p)
...
param(
    ...
    [string]$MaliciousExtensionsUrl = "https://raw.githubusercontent.com/loog4j/Extension-Audit/main/ExtensionIDs/malicious_extensions.txt",
...
"@
    ...
    try {
        # Event ID 9191 chosen to avoid conflicts with common Microsoft event IDs
        # and provide unique identifier for SIEM correlation
        Write-EventLog -LogName $LogName -Source $SourceName -EntryType $EntryType -EventId 9191 -Message $Message -ErrorAction Stop
    } catch {
