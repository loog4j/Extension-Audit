...
index=windows EventCode=9191 earliest=-24h
| stats count by ComputerName, ExtensionId, Name

# Malicious extensions detected
index=windows EventCode=9191 Type=Warning
...
index=windows EventCode=9191 earliest=-7d
| rex field=Message "Name: (?<ExtName>[^\r\n]+)"
| top 10 ExtName
```
...
| **SIEM not collecting** | Verify Event ID 9191 in SIEM agent config, check agent is running |
...
Filter: Event ID = 9191, Source = "Browser Extension Alert"

Export: wevtutil epl Application C:\Logs\Application.evtx /q:"*[System[(EventID=9191)]]"
```
...
- [ ] SIEM confirmed receiving Event ID 9191
