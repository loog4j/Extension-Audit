...
- Verify Event ID 9191 logs are arriving
...
- [ ] Check Event Viewer on test machine for Event ID 9191
...
```spl
index=windows sourcetype="WinEventLog:Application" EventCode=9191 earliest=-24h
| stats count by host
| where count > 0
...
**Symptom:** Task completes (Last Run Result: 0x0) but no Event ID 9191
